...
function isZodDate(schema: z.ZodTypeAny): boolean {
  ...
  if (
    ...
    schema._def.innerType instanceof z.ZodDate
  ) {
    return true;
  }
...
export function formatDateFields<T extends z.ZodRawShape>(
  ...
  for (const key in schemaFields) {
    if (
      Object.prototype.hasOwnProperty.call(schemaFields, key) &&
      isZodDate(schemaFields[key])
    ) {
...
export const formatDateWithOrdinal = (date: Date): string => {
  ...
  return `${dayOfWeek}, ${monthNames[monthIndex]} ${day}${getOrdinalSuffix(
    day
  )}, ${year}`;
};
